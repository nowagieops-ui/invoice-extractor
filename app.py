...
def append_to_tracker(ws, wb, data: dict, row_index: int):
    ...
    wc(ws, r, 8,  f"{data.get('budget_holder_date','')} {bh_s}".strip(), approval_font(bh_s), approval_fill(bh_s), CENTER)
    au_s = data.get("audit_status","")
    wc(ws, r, 9,  data.get("audit_recv"),           sf(),          fill,  CENTER)
    wc(ws, r, 10, f"{data.get('audit_date','')} {au_s}".strip(), approval_font(au_s), approval_fill(au_s), CENTER)
    cc_s = data.get("cost_control_status","")
    wc(ws, r, 11, data.get("cost_control_recv"),    sf(),          fill,  CENTER)
    wc(ws, r, 12, f"{data.get('cost_control_date','')} {cc_s}".strip(), approval_font(cc_s), approval_fill(cc_s), CENTER)
    gm_s = data.get("gm_finance_status","")
    wc(ws, r, 13, data.get("gm_finance_recv"),      sf(),          fill,  CENTER)
    wc(ws, r, 14, f"{data.get('gm_finance_date','')} {gm_s}".strip(), approval_font(gm_s), approval_fill(gm_s), CENTER)
...
Each object must have these exact keys:
{
  ...
  "po_so": "PO or SO number",
  "form_number": "form number or null",
  "invoice_amount": "total amount as number string",
  "received_finance": "DD/MM/YYYY or null",
  "invoice_period": "e.g. JANUARY 2026",
  "budget_holder_name": "name or null",
  "budget_holder_date": "DD/MM/YYYY or null",
  "budget_holder_status": "APPROVED or REJECTED or PENDING",
  "audit_name": "name or null",
  "audit_date": "DD/MM/YYYY or null",
  "audit_status": "APPROVED or REJECTED or PENDING",
  "cost_control_name": "name or null",
  "cost_control_date": "DD/MM/YYYY or null",
  "cost_control_status": "APPROVED or REJECTED or PENDING",
  "gm_finance_name": "name or null",
  "gm_finance_date": "DD/MM/YYYY or null",
  "gm_finance_status": "APPROVED or REJECTED or PENDING",
  "rejection_reason": "reason text or null"
}
Rules: null for missing fields. Dates in DD/MM/YYYY. Status must be APPROVED, REJECTED, or PENDING only."""
...
def extract_with_ai(pdf_text: str) -> list:
    ...
    response = client.models.generate_content(
        ...
        config=gtypes.GenerateContentConfig(temperature=0.1, max_output_tokens=2000)
    )
    ...
    try:
        ...
        response2 = client.models.generate_content(
            model="gemini-3.5-flash",
            contents=f"Return ONLY a valid JSON array. No explanation, no markdown.\n\nExtract from this invoice:\n{pdf_text[:3000]}\n\nKeys: vendor, invoice_number, po_so, invoice_date, received_finance, invoice_period, budget_holder_name, budget_holder_date, budget_holder_status, audit_name, audit_date, audit_status, cost_control_name, cost_control_date, cost_control_status, gm_finance_name, gm_finance_date, gm_finance_status, rejection_reason",
            config=gtypes.GenerateContentConfig(temperature=0, max_output_tokens=1000)
        )
        raw2 = re.sub(r"^```json\s*|^```\s*|\s*```$", "", response2.text.strip()).strip()
        arr2 = re.search(r'\[.*\]', raw2, re.DOTALL)
        obj2 = re.search(r'\{.*\}', raw2, re.DOTALL)
        if arr2: result = json.loads(arr2.group(0))
        elif obj2: result = json.loads(obj2.group(0))
        else: raise ValueError(f"Could not parse Gemini response: {raw2[:200]}")
...
def load_processed():
    if os.path.exists(PROCESSED_LOG):
        with open(PROCESSED_LOG) as f: return set(json.load(f))
    return set()

def save_processed(ids: set):
    with open(PROCESSED_LOG, "w") as f: json.dump(list(ids), f)

def fetch_outlook_pdfs(email_addr, password, server, limit):
    results = []
    ...
    for msg_id in reversed(ids):
        ...
        for part in msg.walk():
            if part.get_content_type() == "application/pdf":
                ...
                with open(save_path, "wb") as f: f.write(part.get_payload(decode=True))
                pdfs_found.append({"path": save_path, "name": fname})
        if pdfs_found:
            results.append({"msg_id": uid, "subject": subject, "sender": sender, "pdfs": pdfs_found})
    mail.logout()
    return results, processed

def process_emails(email_addr, password, server="imap.gmail.com", limit=20):
    emails, processed = fetch_outlook_pdfs(email_addr, password, server, limit)
    wb, ws = get_or_create_workbook()
    last_data_row = 4
    for row in ws.iter_rows(min_row=5):
        if row[0].value is not None: last_data_row = row[0].row
    ...
    for em in emails:
        for pdf_info in em["pdfs"]:
            ...
            if len(pdf_text.strip()) < 50:
                summary.append({"email": em["subject"], "file": pdf_info["name"], "status": "skipped", "reason": "Could not extract text (scanned PDF)"})
                continue
            try:
                records = extract_with_ai(pdf_text)
            except Exception as e:
                summary.append({"email": em["subject"], "file": pdf_info["name"], "status": "error", "reason": str(e)})
                continue
            for rec in records:
                sn = next_sn(ws)
                data = {
                    "sn": sn, "vendor": rec.get("vendor"),
                    ...
                    "audit_recv": rec.get("audit_date"), "audit_date": rec.get("audit_date"),
                    ...
                    "finance_name": None, "finance_date": None,
                    "rejection_reason": rec.get("rejection_reason"),
                }
                append_to_tracker(ws, wb, data, row_index_counter[0])
                row_index_counter[0] += 1
                summary.append({"email": em["subject"], "file": pdf_info["name"], "status": "ok", "sn": sn,
                    "vendor": rec.get("vendor"), "period": rec.get("invoice_period"),
...
def fetch_email():
    ...
    try:
        summary = process_emails(email_addr, password, server, limit)
    except Exception as e:
        return jsonify({"error": str(e), "trace": traceback.format_exc()}), 500
    ok     = [s for s in summary if s["status"] == "ok"]
    errors = [s for s in summary if s["status"] != "ok"]
    return jsonify({"success": True, "processed": ok, "skipped": errors, "total": len(summary)})
...
def preview():
    ...
    if os.path.exists(EXCEL_FILE):
        ...
        if "INVOICE" in wb.sheetnames:
            ws = wb["INVOICE"]
            for row in ws.iter_rows(min_row=5, values_only=True):
                if any(c is not None for c in row): rows.append(row)
